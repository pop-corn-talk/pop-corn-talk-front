...
const CreatePostList = ({ loginCount }) => {
  ...
  async function handleNoticePostSubmit() {
    // 이미지 파일이 선택되었는지 확인
    if (!postImage) {
      console.error("이미지를 선택하세요.");
      return;
    ...
    try {
      ...
      const postResponse = await apiClient.post("/posts/notice", postData);
    } catch (error) {
      if (axios.isAxiosError(error)) {
        message.error(error.message);
      }
        ...
        <div>
          <article id="posts">
            <div className="post-container">
              ...
              <button id="post_submitBtn" onClick={handleNoticePostSubmit}>
                게시글 등록
              </button>
            </div>
          </article>
